...
def file_format_check(ext):
    if ext not in ALLOWED_IMAGE_FORMATS:
        raise Exception(
            "File format must be on one of either: {}".format(
...
class EvokerLite:
    def __init__(
        ...
        bim_path=None,
        bed_path=None,
        snp_posterior_batch_path=None,
        snp_posterior_path=None,
        ...
        self.genotypes = Genotypes(
            bed_path or (bfile_path + ".bed"), n_samples, n_variants
        )
        ...
        if bnt_path:
            ...
            if os.path.isfile(bnt_path):
                ...
                self.intensities = TextIntensity(int_path, self.samples, ukbiobank)
            else:
    ...
    def plot(self, variant_name, batch=None, ellipses=None, ax=None, transform=None):
        ...
        for m in genotype_mapping:
            ...
            if ellipses and code in ellipses:
                ...
                if plot_sexes:
                    ax[0].plot(
                        ellipse_points[:, 0], ellipse_points[:, 1], color="black"
                    )
                    ax[1].plot(
                        ellipse_points[:, 0], ellipse_points[:, 1], color="black"
                    )
    ...
    def save_all_batches(
        ...
        if os.path.exists(directory):
            print(
                "Warning: the directory to hold all of the plots ({}) already exists.".format(
                    directory
                )
            )
        else:
...
def cli():
    ...
    parser.add_argument(
        "-o", "--output", type=str, help="directory to save plots",
    )
    parser.add_argument(
        "-r",
        "--rsids",
